                ...
                if "act" in item.name:  # act or activity
                    ...
                    if lastLeg is not None:
                        if lastLeg.mode == "non_network_walk":
                            ...
                            # FIX: Check bounds before accessing vehicleslist
                            ...
                            if vehIndex < len(vehicleslist):
                                ...
                                # Skip this ride element or handle missing vehicle gracefully
                                if options.verbose:
                                    print(f"Warning: Missing vehicle for person {person.id}, activity {actIndex}", file=sys.stderr)
                                # Option 1: Skip the ride
                                # Option 2: Create a walk instead
                                outf.write('        <walk %s/>\n' % end)
                        if vehIndex < len(vehicleslist):
                            vehIndex += 1
