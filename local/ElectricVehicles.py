...
class ElectricVehicles:
    ...
    def __init__(self, **kwargs):
        ...
        self.soc_cv = kwargs['soc_cv'] if 'soc_cv' in kwargs else 0.95

        if 'batterycapacity_kwh' in kwargs:
            self.modelparameters['ev_packcapacity'] = kwargs['batterycapacity_kwh']
            self.batterycapacity_kWh = kwargs['batterycapacity_kwh']
        ...
        self.ramp_up = False
        self.with_ramp_up = False
        self.with_data_ramp_down = False  # ramp down simulated by model or data inspired model
    ...
    def isvehiclepluggedin(self, simulationtime):
        ...
        if (simulationtime >= self.arrivaltime) and (
                simulationtime < self.arrivaltime + self.modelparameters['ev_setuptime']) and (
                simulationtime <= self.departuretime):
            self.pluggedin = True
            self.readytocharge = False

        elif (simulationtime >= self.arrivaltime + self.modelparameters['ev_setuptime']) and (
                simulationtime <= self.departuretime):
            self.pluggedin = True
    ...
    def getocv(self, soc, **kwargs):
        ...
        if self.modelparameters['ev_batterychemistry'] == 'nmc':
            ...
            rohmpts = np.array(
                [0.00172, 0.00154, 0.00147, 0.00141, 0.001395, 0.00137, 0.00140, 0.00150, 0.00148, 0.00145, 0.00144])
            ...
            socpts = np.linspace(0.0, 1.0, num=ocvpts.size)
            rohmpts = np.array(
                [3.1092, 2.6633, 1.8337, 1.6094, 1.5160, 1.4196, 1.3201, 1.2206, 1.2115, 1.1838, 1.2777]) * 1.0e-3
            ...
            socpts = np.linspace(0.0, 1.0, num=ocvpts.size)
            rohmpts = np.array(
                [5.6123, 3.0693, 2.3927, 2.1543, 2.0235, 2.0338, 2.1051, 2.0431, 2.1884, 2.3383, 2.5148]) * 1.0e-3
            ...
            socpts = np.linspace(0.0, 1.0, num=ocvpts.size)
            rohmpts = np.array(
                [1.7359, 1.6166, 1.5263, 1.4793, 1.6639, 1.6109, 1.6389, 1.5716, 1.6222, 1.0595, 0.4736]) * 1.0e-3
        ...
        Np = math.ceil((self.modelparameters['ev_packcapacity'] * 1.0e3) / (
                    self.modelparameters['ev_nominalvoltage'] * self.modelparameters['ev_cellcapacity']))
    ...
    def chargevehicle(self, simulationtime,  **kwargs):
        ...
        if self.readytocharge and ~np.isnan(self.evse_id) and (not self.chargecomplete):
            ...
            CVvolt = Ns * interpolate.pchip_interpolate(socpts, ocvpts, self.soc_cv)
            ...
            if not power:
                ...
                tempcurrent = (-packocv + math.sqrt(packocv ** 2 + 4.0 * packresistance * power[0] * 1.0e3)) / (
                            2.0 * packresistance)
