...
Analyse-Skript für Logs ohne vorhandene Ladestationen:
- Identifiziert Fahrzeugpositionen mit niedriger SOC (<= threshold %)
- Findet räumliche Hotspots (DBSCAN)
- Erzeugt GeoJSON-Polygone (eingekreiste Cluster) + CSV mit Schätzungen
- Optional: versucht, bei Angabe eines SUMO-Netzes Lane-IDs zu finden und
  schreibt ein SUMO additional-XML (oder kommentierte Koordinaten)

Usage (PowerShell):
python ./code/python/train_from_sumo_log_no_stations.py --log generated_files/logs/sumo_merged_output.csv

Outputs (standard):
- generated_files/logs/no_station_charging_suggestions.csv
- generated_files/logs/no_station_areas.geojson
- generated_charging_no_stations.add.xml
"""
import os
import math
import argparse
...
OUT_CSV = "generated_files/logs/no_station_charging_suggestions.csv"
OUT_GEOJSON = "generated_files/logs/no_station_areas.geojson"
OUT_XML = "generated_charging_no_stations.add.xml"
...
# -----------------------
def main():
    parser = argparse.ArgumentParser(description="Find recommended charging areas from SUMO log when no stations exist")
    parser.add_argument("--log", default=DEFAULT_LOG)
    parser.add_argument("--eps", type=float, default=EPS_METERS)
    parser.add_argument("--min-samples", type=int, default=MIN_SAMPLES)
    parser.add_argument("--threshold", type=float, default=SOC_THRESHOLD, help="SOC percentage threshold (<=)")
    parser.add_argument("--sim-hours", type=float, default=None, help="Total simulation hours (auto-estimate if not set)")
    parser.add_argument("--avg-session-kwh", type=float, default=AVG_SESSION_KWH)
    parser.add_argument("--charger-kw", type=float, default=CHARGER_KW)
    parser.add_argument("--utilization", type=float, default=UTILIZATION)
    parser.add_argument("--net", default=None, help="Optional SUMO net xml for lane attachment")
    parser.add_argument("--out-csv", default=OUT_CSV)
    parser.add_argument("--out-geojson", default=OUT_GEOJSON)
    parser.add_argument("--out-xml", default=OUT_XML)
    args = parser.parse_args()

    df = read_log(args.log)
    total_rows = len(df)
    print(f"Read {total_rows} rows from {args.log}")
    ...
    # select low-SOC rows
    low_df = df[df["soc_percent"] <= args.threshold].copy()
    if len(low_df) == 0:
        print(f"No rows with soc_percent <= {args.threshold} found. Nothing to do.")
        return
    print(f"Low-SOC rows (<= {args.threshold}%): {len(low_df)}")

    coords = low_df[["x","y"]].to_numpy()
    # cluster
    labels = cluster_low_soc_points(coords, eps=args.eps, min_samples=args.min_samples)
    ...
    net = None
    if args.net and os.path.exists(args.net) and HAS_SUMOLIB:
        try:
            net = sumolib.net.readNet(args.net)
            print("Loaded SUMO network:", args.net)
        except Exception as e:
            print("Could not load net:", e)
            net = None

    for i, c in enumerate(sorted(clusters.values(), key=lambda x: x["count"], reverse=True)):
        est_chargers = estimate_chargers(c["count"], sim_hours, avg_session_kwh=args.avg_session_kwh,
                                         charger_kw=args.charger_kw, utilization=args.utilization)
        csv_rows.append({
    ...
    print("Wrote GeoJSON:", args.out_geojson)
    ...
    with open(args.out_xml, "w", encoding="utf-8") as f:
        ...
        for r in csv_rows:
            ...
            start_pos = 0.0
            end_pos = 2.0
            power_w = int(args.charger_kw * 1000)
            if lane_id:
        ...
        f.write("</additional>\n")
    print("Wrote XML (or commented suggestions):", args.out_xml)
